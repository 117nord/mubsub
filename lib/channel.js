...
Channel.prototype.create = function() {
    ...
    function cb(err, results) {
        if (err && err.message === 'collection already exists') {
            // The mongodb module should handle the collection already exists error internally
            // but a race condition in the driver can cause it to be returned.
            // In that case we simply retry it.
            return process.nextTick(function () {
                create();
            });
        }

        self.collection.resolve(err, results);
    }
